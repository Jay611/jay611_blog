...
const authCtrl = {
  ...
  refreshToken: async (req: Request, res: Response) => {
    try {
      ...
      if (!decoded.id) return res.status(500).json({ msg: 'Please login now!' })

      const user = await Users.findById(decoded.id).select('-password')
      if (!user)
        return res.status(500).json({ msg: 'This account does not exist.' })
  ...
  smsVerify: async (req: Request, res: Response) => {
    try {
      ...
      const password = phone + `${process.env.PASSWORD_SUFFIX}`
      const passwordHash = await bcrypt.hash(password, 12)
...
const loginUser = async (user: IUser, password: string, res: Response) => {
  ...
  const access_token = generateAccessToken({ id: user._id })
  const refresh_token = generateRefreshsToken({ id: user._id })
...
const registerUser = async (user: IUserParams, res: Response) => {
  ...
  const refresh_token = generateRefreshsToken({ id: newUser._id })
